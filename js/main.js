window.addEventListener("load", function() {
    ...
    const qCodesMenu = document.getElementById("qCodesMenu");

    // function displayQcode(){
    //   qCodeKeys.forEach((q, i) => {
    //     qCodesList.innerHTML += '<a class="dropdown-item" href="#">' + q + ' : ' + questionCode[q] + '</a>';
    //   });
    //
    // }

    function displayQcodeMenu() {
        qCodeKeys.forEach((q, i) => {
            qCodesMenu.innerHTML += '<option value="' + i + '">' + q + ' : ' + questionCode[q] + '</option>';
        });
    ...
    function displayEcode() {
        eCodeKeys.forEach((e, i) => {
            eCodesList.innerHTML += '<option>' + e + ' : ' + equipmentCode[e] + '</option>';
        });
